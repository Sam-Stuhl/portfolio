...
rather than as the logo. `build`-time they are not touched; `src/favicon.svg` and
`src/favicon-32.png` are generated by hand from a 16-unit grid (see the comment
in the SVG) that keeps every edge on a whole pixel at 16px and puts the mark on a
white plate, which is the composition the designer's own 180px
`apple-touch-icon.png` already uses. That icon is served exactly as delivered.
